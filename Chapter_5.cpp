...
bool search(BinaryTree<T> *tree, const T &key)
{
  ...
  return (search(new BinaryTree<T>(tree->Root()->leftChild()), key) || search(new BinaryTree<T>(tree->Root()->rightChild()), key)) == true;
}
...
int main()
{
  ...
  cout << search(new BinaryTree<int>(root), 2) << endl;
}